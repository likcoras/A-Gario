...
public class ConnectHook extends ListenerAdapter<AgarBot> {
    private static final String API_DATA = "http://connect.agariomods.com/json/api.php?action=GSFUC&username=";
    private static final String NO_USER = Utils.addFormat("&04No user named &r");
    private static final String NO_IMG = Utils.addFormat("&04No image uploaded by &r");
    private static final String INFO = Utils.addFormat("&03User: &r%s | &03Skin: &rhttps://connect.agariomods.com/img_%s.png");
    
    @Override
    public void onGenericMessage(GenericMessageEvent<AgarBot> event) throws IOException {
        AgarBot bot = event.getBot();
        String message = event.getMessage();
        if (!Utils.isTrigger(message, "@con ") || !bot.getSpam().check(event.getUser())) {
            return;
        }
